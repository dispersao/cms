...
module.exports = {
  async create(ctx) {
    if (Array.isArray(ctx.request.body)) {
      ...
      return await Promise.all(
        ctx.request.body.map(strapi.services.sessioncontent.create)
      )
    } else {
  ...
  async stateUpdate(ctx) {
    const states = ['pending', 'published']
    ...
    if (
      /*entity.state === states[0] &&*/ ctx.request.body.state === states[1] &&
      (entity.post || entity.comment)
    ) {
      sendMessages(entity)
    }

    entity = await strapi.services.sessioncontent.update(
      ctx.params,
      ctx.request.body
    )
    const model = strapi.models.sessioncontent
